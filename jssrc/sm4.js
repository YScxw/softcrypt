...
function GET_ULONG_BE(n, b, i) {
    n = leftMove(b[i], 24) | leftMove(b[i + 1], 16) |
        leftMove(b[i + 2], 8) | leftMove(b[i + 3], 0)
...
function PUT_ULONG_BE(n, b, i) {
    b[i] = rightMove(n, 24)
    b[i + 1] = rightMove(n, 16)
    b[i + 2] = rightMove(n, 8)
    b[i + 3] = rightMove(n, 0)
}

function leftMove(a, step) {
    return a << step
}

function rightMove(a, step) {
    if(a >= 0) {
        return a >> step
    } else {
...
function SHL(x, n) {
    return leftMove(x & 0xFFFFFFFF, n)
}

function ROTL(x, n) {
    var a = SHL(x, n)
    var b = rightMove(x, 32 - n)
...
SM4.prototype.SM4CryptECB = function (szData, sCryptFlag, szSM4Key) {
    ...
    var lpbKey = new Buffer(szSM4Key, 'hex');
    ...
    var lpbData = new Buffer(szData, 'hex')
    var pbyCryptResult = new Buffer(len / 2)
...
SM4.prototype.SM4MACGenerated = function (szData, szSM4Key) {
    if (szSM4Key.length !== 32) {
        console.log("传入密钥[" + szSM4Key + "]长度不为32位");
        return "";
    }
    ...
    for (var i = 0; i < count; i++) {
        ...
        szResult = SM4.prototype.SM4CryptECB(macString, 1, szSM4Key);
    }
    return szResult
}
